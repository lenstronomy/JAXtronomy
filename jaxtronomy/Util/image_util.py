...
def add_layer2image(grid2d, x_pos, y_pos, kernel, order=1):
    ...
    """
    if order != 1:
        raise ValueError(f"spline interpolation order {order} is not supported in jaxtronomy")
...
def add_layer2image_int(grid2d, x_pos, y_pos, kernel):
    ...
    out_of_bounds = jnp.where(x_int >= image_cols + kernel_x_radius, True, out_of_bounds)
    out_of_bounds = jnp.where(y_int >= image_rows + kernel_y_radius, True, out_of_bounds)
    ...
    padded_image = jnp.pad(grid2d, pad_width=((kernel_y_radius, kernel_y_radius), (kernel_x_radius, kernel_x_radius)), mode="constant")
    ...
    # Unpads the image
    return lax.fori_loop(0, k_rows, body_fun, padded_image)[kernel_y_radius:-kernel_y_radius, kernel_x_radius:-kernel_x_radius]
...
def shift(input, shift):
    ...
    if shift.shape == ():
        ...
        raise ValueError("shift must be either a float or a list/tuple/array of floats with length 2")
    ...
    def shift_down(new_array, old_array, shift):
        ...
        def body_fun(i, new_array):
            ...
            value = old_array.at[row_index].get() * (1. - shift) + old_array.at[row_index-1].get() * shift
