...
class SinglePlaneLOS(SinglePlane):
    ...
    effects as presented by Fleury et al. in 2104.08883.
    ...
    def __init__(
        ...
        See SinglePlane() docstring for other documentation.
        """
        ...
        self.los = lens_class(
            self._los_model,
            profile_kwargs=profile_kwargs_list[index_los],
        )
        ...
        self._main_lens = SinglePlane(
            ...
            lens_redshift_list=lens_redshift_list_wo_los,
    ...
    def split_lens_los(self, kwargs):
        ...
        correspond to the line-of-sight corrections (kwargs_los).
    ...
    def fermat_potential(
        ...
        A_od_tsp = jnp.transpose(A_od)
        A_ds_inv = jnp.linalg.inv(A_ds)
        A_os_inv = jnp.linalg.inv(A_os)

        # the composite amplification matrices
        A_LOS = jnp.dot(jnp.dot(A_od_tsp, A_ds_inv), A_os)
