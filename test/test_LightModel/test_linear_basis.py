...
class TestLinearBasis(object):
    def setup_method(self):
        ...
        self.linear_basis = LinearBasis(light_model_list=["SERSIC", "MULTI_GAUSSIAN", "SHAPELETS"], profile_kwargs_list=[{}, {}, {"n_max": n_max}])
        self.linear_basis_ref = LinearBasis_ref(light_model_list=["SERSIC", "MULTI_GAUSSIAN", "SHAPELETS"])
    ...
    def test_functions_split(self):
        ...
        response_ref, n_ref = self.linear_basis_ref.functions_split(x, y, self.kwargs_list)
        npt.assert_allclose(response, response_ref, atol=1e-12, rtol=1e-12)
        assert n == n_ref == 14

    def test_num_param_linear(self):
        assert self.linear_basis.num_param_linear(self.kwargs_list) == self.linear_basis_ref.num_param_linear(self.kwargs_list) == 14

    def test_num_param_linear_list(self):
        param_list = self.linear_basis.num_param_linear(self.kwargs_list)
        param_list_ref = self.linear_basis_ref.num_param_linear(self.kwargs_list)
    ...
    def test_update_linear(self):
        ...
        kwargs_list_ref, i_new_ref = self.linear_basis_ref.update_linear(param, i, self.kwargs_list)
    ...
    def test_check_positive_flux_profile(self):
        linear_basis = LinearBasis(light_model_list=["SERSIC", "SERSIC_ELLIPSE"])
        linear_basis_ref = LinearBasis_ref(light_model_list=["SERSIC", "SERSIC_ELLIPSE"])
        kwargs_list = [{"amp": 0.5}, {"amp": -1}]
