...
class TestPointSource(object):
    def setup_method(self):
        ...
        self.ps = PointSource(point_source_type_list=point_source_type_list, lens_model=lensmodel, fixed_magnification_list=[False, False, True])

        lensmodel_ref = LensModel_ref(lens_model_list=lens_model_list)
        self.ps_ref = PointSource_ref(point_source_type_list=point_source_type_list, lens_model=lensmodel_ref, fixed_magnification_list=[False, False, True])
    ...
    def test_source_position(self):
        x_source_list, y_source_list = self.ps.source_position(self.kwargs_ps, self.kwargs_lens)
        x_source_list_ref, y_source_list_ref = self.ps_ref.source_position(self.kwargs_ps, self.kwargs_lens)
        for i in range(len(x_source_list)):
            print(f"testing point_source_type_list {i}")
            npt.assert_allclose(x_source_list[i], x_source_list_ref[i], atol=1e-8, rtol=1e-8)
            npt.assert_allclose(y_source_list[i], y_source_list_ref[i], atol=1e-8, rtol=1e-8)

    def test_image_position(self):
        x_image_list, y_image_list = self.ps.image_position(self.kwargs_ps, self.kwargs_lens)
        x_image_list_ref, y_image_list_ref = self.ps_ref.image_position(self.kwargs_ps, self.kwargs_lens)
        for i in range(len(x_image_list)):
            print(f"testing point_source_type_list {i}")
            npt.assert_allclose(x_image_list[i], x_image_list_ref[i], atol=1e-8, rtol=1e-8)
            npt.assert_allclose(y_image_list[i], y_image_list_ref[i], atol=1e-8, rtol=1e-8)

    def test_point_source_list(self):
        ra_array, dec_array, amp_array = self.ps.point_source_list(self.kwargs_ps, self.kwargs_lens)
        ra_array_ref, dec_array_ref, amp_array_ref = self.ps_ref.point_source_list(self.kwargs_ps, self.kwargs_lens)
    ...
    def test_check_image_position(self):
        within_tolerance = self.ps.check_image_positions(self.kwargs_ps, self.kwargs_lens)
        within_tolerance_ref = self.ps_ref.check_image_positions(self.kwargs_ps, self.kwargs_lens)
        assert within_tolerance == within_tolerance_ref

        within_tolerance = self.ps.check_image_positions(self.kwargs_ps, self.kwargs_lens, tolerance=30)
        within_tolerance_ref = self.ps_ref.check_image_positions(self.kwargs_ps, self.kwargs_lens, tolerance=30)
        assert within_tolerance == within_tolerance_ref
    ...
    def test_set_amplitudes(self):
        ...
        assert amp_list[0] == kwargs_new[0]["point_amp"]
        assert amp_list[1] == kwargs_new[1]["point_amp"]
        assert amp_list[2] == kwargs_new[2]["source_amp"]

    def test_raises(self):
        npt.assert_raises(ValueError, PointSource, ["UNLENSED"], index_lens_model_list=[[0]])
...
class TestPointSourcewithBands(TestPointSource):
    def setup_method(self):
        lens_model_list = ["EPL", "SIS"]
        point_source_type_list = ["LENSED_POSITION", "UNLENSED", "LENSED_POSITION"]

        lensmodel = LensModel(lens_model_list=lens_model_list)
        self.ps = PointSource(point_source_type_list=point_source_type_list, lens_model=lensmodel, fixed_magnification_list=[False, False, True], index_lens_model_list=[[0], [1]], point_source_frame_list=[[0, 0, 0], None, [1, 1, 1]])

        lensmodel_ref = LensModel_ref(lens_model_list=lens_model_list)
        self.ps_ref = PointSource_ref(point_source_type_list=point_source_type_list, lens_model=lensmodel_ref, fixed_magnification_list=[False, False, True], index_lens_model_list=[[0], [1]], point_source_frame_list=[[0, 0, 0], None, [1, 1, 1]])
