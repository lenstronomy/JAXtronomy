...
from lenstronomy.PointSource.Types.lensed_position import LensedPositions as LensedPositions_ref
...
class TestLensedPosition(object):
    def setup_method(self):
        ...
        self.kwargs_lens = [{"theta_E": 2.3874, "center_x": 0.1345, "center_y": -0.23847}, {"theta_E": 1.3874, "center_x": 0.2345, "center_y": 0.123847}]
        ...
        self.kwargs_fixed_mag = {"source_amp": 2, "ra_image": [0, 1.2], "dec_image": [0, 0]}
        ...
        self.kwargs = {"point_amp": [2, 1], "ra_image": [0.32, 1.3], "dec_image": [0.345, -0.923]}
    ...
    def test_image_position(self):
        ...
        x_img_ref, y_img_ref = self.ps_fixed_mag_ref.image_position(self.kwargs_fixed_mag)
        npt.assert_allclose(x_img, x_img_ref, rtol=1e-10, atol=1e-10)
        npt.assert_allclose(y_img, y_img_ref, rtol=1e-10, atol=1e-10)

        npt.assert_raises(ValueError, self.ps.image_position, self.kwargs, additional_images=True)


    def test_source_position(self):
        x_src, y_src = self.ps.source_position(self.kwargs, kwargs_lens=self.kwargs_lens)
        x_src_ref, y_src_ref = self.ps_ref.source_position(self.kwargs, kwargs_lens=self.kwargs_lens)
        npt.assert_allclose(x_src, x_src_ref, rtol=1e-10, atol=1e-10)
        npt.assert_allclose(y_src, y_src_ref, rtol=1e-10, atol=1e-10)

        x_src, y_src = self.ps_fixed_mag.source_position(self.kwargs_fixed_mag, kwargs_lens=self.kwargs_lens)
        x_src_ref, y_src_ref = self.ps_fixed_mag_ref.source_position(self.kwargs_fixed_mag, kwargs_lens=self.kwargs_lens)
        npt.assert_allclose(x_src, x_src_ref, rtol=1e-10, atol=1e-10)
        npt.assert_allclose(y_src, y_src_ref, rtol=1e-10, atol=1e-10)
    ...
    def test_source_amplitude(self):
        ...
        amp_ref = self.ps_ref.source_amplitude(self.kwargs, kwargs_lens=self.kwargs_lens)
        npt.assert_allclose(amp, amp_ref, rtol=1e-10, atol=1e-10)

        amp = self.ps_fixed_mag.source_amplitude(self.kwargs_fixed_mag, kwargs_lens=None)
        amp_ref = self.ps_fixed_mag_ref.source_amplitude(self.kwargs_fixed_mag, kwargs_lens=None)
        npt.assert_allclose(amp, amp_ref, rtol=1e-10, atol=1e-10)
