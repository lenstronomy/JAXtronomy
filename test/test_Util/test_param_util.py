...
def test_cart2polar():
    ...
    assert r == jnp.sqrt(2)  # radial part
    assert phi == jnp.arctan(1)
    # array of 2d coordinates
    x = jnp.array([1, 2])
    y = jnp.array([1, 1])

    r, phi = param_util.cart2polar(x, y, center_x, center_y)
    assert r[0] == jnp.sqrt(2)  # radial part
    assert phi[0] == jnp.arctan(1)
...
def test_ellipticity2phi_q():
    ...
    e1 = jnp.array([0.3, 0.9])
    e2 = jnp.array([0.0, 0.9])
    phi, q = param_util.ellipticity2phi_q(e1, e2)
...
def test_displace_eccentricity():
    ...
    xt1 *= jnp.sqrt(q)
    xt2 /= jnp.sqrt(q)
    npt.assert_array_almost_equal(x_, xt1, decimal=8)
    npt.assert_array_almost_equal(y_, xt2, decimal=8)
